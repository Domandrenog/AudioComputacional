...
import threading
import queue
from music21 import stream, note, midi, environment, configure

# Configure music21 environment
environment.set('musicxmlPath', 'C:\\Program Files\\MuseScore 4')  # Substitua pelo caminho correto do seu programa MuseScore
configure.run()
...
def process_audio_data(fs, audio_queue, event):
    ...
    finalmusic = stream.Score()
    finalpart = stream.Part()
    # Create a new part for each iteration
    part = stream.Part()
    
    while time.time() - start_time < 20:  # Wait for 20 sec
        ...
        player = midi.realtime.StreamPlayer(part)
        player.play()
        if part:
            part.pop(-1)

    finalmusic.append(finalpart)
    finalmusic.show()  # Mostra só uma nota
